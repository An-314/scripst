...
| `countblock-lang` | `str` | `"en"` | 内置 countblock 名称的语言（`"en"` 或 `"zh"`） |
...
内置块名默认使用英文。在 `scripst.with(...)` 中设置 `countblock-lang: "zh"` 即可显示中文名称。自定义块既可以使用固定的 `str`/`content` 名称，也可以传入 `(en: "Assumption", zh: "假设")` 这样的多语言字典；缺少所选语言时会回退到 `en`。
